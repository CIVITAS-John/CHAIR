...
export class Visualizer {
    ...
    public constructor(Container: Cash) {
        ...
        // Initialize the SVG
        const Root = d3.select(Container.get(0) ?? null).attr("style", "background-color: #290033");
    ...
    public ChosenCallbacks = new Map<string, (Node: Node<unknown>, Status: boolean) => void>();
    ...
    public RegisterChosenCallback<T>(Name: string, Callback: (Node: Node<T>, Status: boolean) => void) {
        this.ChosenCallbacks.set(Name, Callback as (Node: Node<unknown>, Status: boolean) => void);
    }
    ...
    public FocusOnNode(Element: SVGElement | null) {
        if (!Element) {
            return;
        }
    ...
    public RenderCodes(Alpha: number) {
        ...
        if (Alpha <= 0.3) {
            AllLabels.join(
                ...
                .attr("x", (Node) => (Node.x ?? NaN) + ((Node as Node<unknown>).Size ?? NaN) * 0.5 + 0.25)
    ...
    public GenerateLayout<T>(Graph: Graph<T>, Renderer: (Alpha: number) => void) {
        ...
        this.Simulation.nodes(Graph.Nodes)
            ...
            .force(
                "link",
                ForceLink.links(Graph.Links.filter((Link) => (Link.VisualizeWeight ?? NaN) >= 0.1))
