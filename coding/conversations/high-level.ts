...
export abstract class HighLevelAnalyzerBase extends ConversationAnalyzer {
    ...
    public ParseResponse(Analysis: CodedThread, Lines: string[], Messages: Message[], _ChunkStart: number): Promise<number> {
        let Category = "";
        let Position = "";
        ...
        for (let Line of Lines) {
            if (Line.startsWith("* Summary")) {
                ...
                if (CurrentCode) {
                    ...
                    const Example = Message ? AssembleExampleFrom(Message) : Line;
