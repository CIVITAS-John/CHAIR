...
File.writeFileSync(GetMessagesPath(Group, `Conversations/${Name.replace(".json", "")}-${LLMName}.json`), JSON.stringify(Result, null, 4));
// Write the result into an Excel file
var Book = ExportConversationsForCoding(Object.values(Conversations), Result);
await Book.xlsx.writeFile(GetMessagesPath(Group, `Conversations/${Name.replace(".json", "")}-${LLMName}.xlsx`));
process.exit(0);