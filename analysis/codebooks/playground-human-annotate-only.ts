...
await UseLLMs(async () => {
    ...
    var Consolidator = new PipelineConsolidator(
        ...
        // Merge definitions
        new RefineMerger({ Threshold: 0.4, Penalty: 0, UseDefinition: false }),
        new RefineMerger({ Threshold: 0.4, Penalty: 0, Looping: true }),
    );
    Consolidator.Name = "annotated";
    for (var Human of Humans) {
        await ConsolidateConversations(Consolidator, "Coded Dataset 1", "0~16-gpt-3.5-turbo.json", "human", Human, false);
    }
}, "llama3-70b");
