...
export class DatasetSection extends Panel {
            ...
            this.BuildTable(
                ...
                (Row, [Key, Value]) => {
                    ...
                    Summary.append($(`<h4></h4>`).text(Key)).on("click", (Event) => {
                        if (Event.shiftKey) this.Visualizer.SetFilter(false, new DatasetFilter(), Key, Event.shiftKey);
    ...
    public ShowDataset(Name: string, Dataset: Record<string, DataChunk<DataItem>>) {
        ...
        if (!this.Visualizer.IsFilterApplied("Dataset", Name)) this.Visualizer.SetFilter(false, new DatasetFilter(), Name);
        // Show the component
        this.SetRefresh(() => {
            this.Container.empty();
            // Show the title
            this.Container.append(
                $(`<h3>${Name} (${Object.keys(Dataset).length} Chunks)</h3>`).prepend(
                    this.BuildReturn(() => {
                        if (this.Visualizer.IsFilterApplied("Dataset", Name)) this.Visualizer.SetFilter(false, new DatasetFilter());
                        this.ShowDatasets();
                    }),
                ),
            );
