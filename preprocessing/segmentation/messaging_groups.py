...
if len(sys.argv) < 2:
    ...
    sys.path.append("../")
    file_path = (
        config["DATASET_PATH"] or ""
    ) + r"\Messaging Groups\Users of Physics Lab (Group 1)\Messages.csv"
else:
    ...
    with open(file_path, "r", encoding="utf-8") as csv_file:
        ...
        for row in csv_reader:
            ...
            if csv_reader.line_num == 1:
                continue
            _ids.append(int(row[0]))
            # Convert to number and add to the list
            _timestamps.append(int(int(row[3]) / 1000))

    # Convert lists to numpy arrays
    timestamps = np.array(_timestamps)
    ids = np.array(_ids)
