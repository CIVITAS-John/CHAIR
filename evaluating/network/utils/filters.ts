...
export abstract class FilterBase<TNode, TParameter> {
    ...
    public GetParameterNames(_Visualizer: Visualizer): string[] {
        return this.Parameters.map((Parameter) => JSON.stringify(Parameter));
    }
            ...
            this.ExampleIDs = Array.from(
                new Set(
                    Object.entries(Sources)
                        ...
                        .flatMap(([_Key, Value]) => Object.values(Value).flatMap((Item) => Item.AllItems ?? []))
    ...
    public GetParameterNames(_Visualizer: Visualizer): string[] {
        return this.Parameters.map((Parameter) => {
            if (!Parameter.Representative) {
                throw new Error("Parameter has no representative node.");
            }
            return Parameter.Representative.Data.Label;
        });
    }
    /** Filter: The filter function. */
