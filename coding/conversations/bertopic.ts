...
export default class BertopicAnalyzer extends ConversationAnalyzer {
    ...
    public async BatchPreprocess(Conversations: Conversation[], Analyzed: CodedThread[]): Promise<void> {
        ...
        var Topics: Record<number, { IDs: number[], Probabilities: number[] }> = {};
...
===`.trim();
            ...
            var Response = await RequestLLMWithCache([
                ...
                new HumanMessage(Examples.map(Message => `- ${BuildMessagePrompt(Message, undefined, undefined, true)}`).join("\n"))
            ], this.Name, this.BaseTemperature, false);
            // Parse the response
            var Definition = ""; var Phrase = "";
