...
import type { CodedThread, CodedThreads, DataChunk, DataItem } from "./utils/schema.js";
...
export async function ProcessDataset<T extends DataItem>(Analyzer: Analyzer<DataChunk<T>, T, CodedThread>, Group: string, FakeRequest = false) {
    ...
    for (const [Name, Chunk] of Object.entries(Dataset.Data)) {
        const Result = await AnalyzeChunk(Analyzer, Chunk, { Threads: {} }, FakeRequest);
...
export async function AnalyzeChunk<T extends DataItem>(
    ...
    for (const [Key, Chunk] of Object.entries(Chunks)) {
        if (!Chunk.AllItems) {
            throw new Error(`AllItems is not defined in chunk ${Key}.`);
        }
    ...
    await Analyzer.BatchPreprocess(
        ...
        Keys.map((Key) => Analyzed.Threads[Key]),
    );
...
export async function LoopThroughChunks<TUnit, TSubunit, TAnalysis>(
    ...
    for (let I = 0; I < Analyzer.MaxIterations; I++) {
        ...
        while (Cursor < Filtered.length) {
            ...
            while (Tries <= 4) {
                // Get the chunk size
                const _ChunkSize = Analyzer.GetChunkSize(Math.min(MaxItems, Filtered.length - Cursor), Filtered.length - Cursor, I, Tries);
