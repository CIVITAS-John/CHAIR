...
monthly_data['MonthID'] = monthly_data['Month'].dt.month

# Export the data to a CSV file
output_file = file_path1.replace('Conversations.json', '../Monthly-Data.csv')
...
ax1.bar(monthly_data['Month'].astype(str), monthly_data['FirstSeen'], label='Newcomers', color='cyan', alpha=0.8)
ax1.bar(monthly_data['Month'].astype(str), monthly_data['Messages'] - monthly_data['JC Messages'], label='Total Messages (-JC)', color=color, alpha=0.8, bottom=monthly_data['FirstSeen'])
...
ax2.plot(monthly_data['Month'].astype(str), monthly_data['JC Messages'] / monthly_data['Messages'] * 100, label='Message %', marker='o', color=color)
ax2.plot(monthly_data['Month'].astype(str), monthly_data['JC Conversations'] / monthly_data['Conversations'] * 100, label='Conversations %', marker='x', color=color)
...
ax1.set_xlabel('Month')
ax1.set_ylabel('Messages', color=color)
ax1.bar(monthly_data['Month'].astype(str), (monthly_data['Messages'] - monthly_data['JC Messages']) / (monthly_data['Participants'] - 1), label='Messages / Participants (-JC)', color=color, alpha=0.8)
ax1.tick_params(axis='y', labelcolor=color)
ax1.legend(loc='upper left')
