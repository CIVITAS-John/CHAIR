...
import numpy as np
from embedding import cpus, dims, embeddings, items
...
min_samples = int(sys.argv[5]) if len(sys.argv) > 5 else 1
...
db = DBSCAN(eps=epsilon, min_samples=min_samples, metric="precomputed", n_jobs=cpus)
db.fit(distances)
