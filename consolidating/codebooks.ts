...
import { Code, CodedThreads, Codebook, DataItem, DataChunk } from "../utils/schema.js";
...
export function MergeCodebook(Analyses: CodedThreads) {
    ...
    for (const [Key, Analysis] of Object.entries(Analyses.Threads)) {
        for (const [Code, Value] of Object.entries(Analysis.Codes)) {
            ...
            if (Value.Examples?.length ?? 0 > 0) Current.Examples = [...new Set([...Current.Examples!, ...Value.Examples!])];
            if (Value.Definitions?.length ?? 0 > 0) Current.Definitions = [...new Set([...Current.Definitions!, ...Value.Definitions!])];
            if (Value.Categories?.length ?? 0 > 0) Current.Categories = [...new Set([...Current.Categories!, ...Value.Categories!])];
...
export function MergeCodebooks(Codebooks: Codebook[], WithReference: boolean = false) {
    ...
    for (var [Index, Codebook] of Codebooks.entries()) {
        for (var [Label, Code] of Object.entries(Codebook)) Code.Owners = [];
    }
    ...
    for (var [Index, Codebook] of Codebooks.entries()) {
        for (var [Label, Code] of Object.entries(Codebook)) {
            ...
            if (WithReference) {
                ...
                else MergeCodes(Codes.get(NewLabel)!, Code);
            }
        }
    }
...
export async function ConsolidateCodebook<TUnit>(
    ...
    await LoopThroughChunks(
        ...
        async (Currents, ChunkStart, IsFirst, Tries, Iteration) => {
            ...
            // Parse the response
            var Result = await Consolidator.ParseResponse(
                Analyses,
                Response.split("\n").map((Line) => Line.trim()),
                Currents,
                ChunkStart,
...
export function MergeCodes(Parent: Code, Code: Code) {
    ...
    if (Parent.Owners || Code.Owners) Parent.Owners = Array.from(new Set((Parent.Owners ?? []).concat(Code.Owners ?? [])));
    Code.Label = "[Merged]";
    return Parent;
}
...
export function MergeCodesByCluster(Clusters: Record<number, ClusterItem[]>, Codes: Code[]): Record<string, Code> {
    ...
    for (var Key of Object.keys(Clusters)) {
        ...
        var BestCode = Clusters[ClusterID].sort((A, B) => B.Probability - A.Probability)
        ...
        for (var Item of Clusters[ClusterID]) {
            ...
            if (ClusterID == -1) {
                ...
                if (BestCode.OldLabels!.includes(Code.Label) != true) BestCode.OldLabels!.push(Code.Label);
...
export function UpdateCodes(Codebook: Codebook, NewCodes: Code[], Codes: Code[]): Record<string, Code> {
    ...
    for (var I = 0; I < Codes.length; I++) {
        ...
        if (NewLabel != Codes[I].Label) {
            ...
            if (!Alternatives.includes(Codes[I].Label)) Alternatives.push(Codes[I].Label);
            if (Alternatives.includes(NewLabel)) Alternatives = Alternatives.filter((Alternative) => Alternative != NewLabel);
    ...
    for (var I = 0; I < Categories.length; I++) {
        ...
        for (var Code of Codes) {
            if (Code.Categories?.includes(Category)) {
                ...
                if (Code.Categories?.includes(NewCategory) !== true) Code.Categories.push(NewCategory);
            }
        }
    }
}
...
export function AssignCategoriesByCluster(Clusters: Record<number, ClusterItem[]>, Codes: Code[]): Record<string, Code[]> {
    ...
    for (var Key of Object.keys(Clusters)) {
        ...
        if (ClusterID != -1) Results[ClusterName] = Items;
    }
    return Results;
}
...
export function MergeCategoriesByCluster(
    ...
    for (var Key of Object.keys(Clusters)) {
        ...
        console.log(
            "Merging categories: " +
                Clusters[ClusterID].map((Item) => `${Categories[Item.ID]} with ${(Item.Probability * 100).toFixed(2)}%`).join(", "),
        );
        for (var Code of Codes) {
