...
export class Dialog extends Panel {
    ...
    private ShowPanel(Panel: Cash) {
        ...
        (Content.get(0) as HTMLElement).scrollTop = 0;
    ...
    public ValidateCoverageByCodes(ScrollTo?: string) {
        ...
        // Get the codebooks
        const Indexes =
            (this.Visualizer.GetFilter("Owner")?.Parameters as number[] | undefined) ??
        ...
        this.BuildTable(
            ...
            (Row, Node, Index) => {
                ...
                // Show the label
                Row.append(
                    $(`<td class="actionable"><h4>${Index + 1}. ${Node.Data.Label}</h4></td>`).on("click", () => {
                        this.Visualizer.PushState("validate-coverage-by-codes", () => {
                ...
                for (const Codebook of Indexes) {
                    ((Index) => {
                        ...
                        Select.on("change", () => {
                            this.VerifiedOwnerships.get(Node.ID)?.set(Index, parseInt(Select.val() as string));
                        ...
                        for (const Original of Related) {
                            for (const Code of FindOriginalCodes(Codebook, Original, Index)) {
                                ((Original, Code) => {
                                    const Link = $('<a href="javascript:void(0)"></a>').text(Code.Label).appendTo(Cell);
                                    Link.on("click", () => {
                                        this.Visualizer.PushState("validate-coverage-by-codes", () => {
                                            this.ValidateCoverageByCodes(Node.Data.Label);
                                        });
        ...
        Title.append(
            $('<span><a href="javascript:void(0)" class="copy">Save to Clipboard</a></span>').on("click", () => {
                ...
                Codes.forEach((Node) => {
                    const Owners = this.VerifiedOwnerships.get(Node.ID) ?? new Map<number, number>();
