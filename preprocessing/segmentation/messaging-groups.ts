...
async function SeperateMessages(Source: string, Prefix: string) {
    ...
    var Python = spawnAsync("python", ["preprocessing/segmentation/messaging_groups.py", GetMessagesPath(Source, "Messages.csv")]);
    ...
    var Indexes = File.readFileSync(GetMessagesPath(Source, `Messages.Groups.csv`), "utf-8")
        .split("\n")
    ...
    for (var I = 0; I < Messages.length; I++) {
        ...
        if (Message.Time > CurrentConversation.End) CurrentConversation = Conversations[++ConversationIndex];
